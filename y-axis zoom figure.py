...
def second_to_str(t):
    result = [t * 1000]  # start point
    for val in [1000, 60, 60]:  # division checks
        result = list(divmod(result[0], val)) + result[1:]
    return "%d:%02d:%02d.%02d" % tuple(result)
...
def colours(c):
    ...
    if type(c) == str:
        if colourdict.has_key(c) == False:
            sys.exit(
                'The supplied colour "%s" is not valid.\nPlease look at the available colours and enter a valid colour call key.' % (
                c))
        c = colourdict[c]
    if type(c) != str:
        if len(c) != 3:
            sys.exit(
                'The supplied colour tuple "%s" is too long or short.\nThe RGB colour tuple should have three values (in the range 0-255) separated by commas within round brackets.\ne.g. "(79, 129, 189)" is R = 79, G = 129, B = 189' % (
                str(c)))
        for i in range(3):
            if type(c[i]) != int:
                sys.exit(
                    'One of the values in your colour tuple is not an integer ("%s").\nPlease correct the value.' % (
                    str(c[i])))
            if c[i] > 255 or c[i] < 0:
                sys.exit(
                    'One of the values in your colour tuple is outside of the valid RGB range of 0-255 ("%s").\nPlease check and correct the value' % (
                    str(c[i])))
    out = []
...
def renderplot(ymax, timeslot, frame):
        ...
        for ind1, species in enumerate(data[sheet]):  # for each species
            if data[sheet][species][1] >= 1:  # if alpha trigger indicates fully visible
                if species != xname[sheet]:  # if species is not x axis
                    pl.plot(data[sheet][xname[sheet]][2], data[sheet][species][2],
                            color=colours(data[sheet][species][0]), alpha=1, linewidth=lw)
            if data[sheet][species][1] == 0:  # if alpha trigger indicates fade
                if species != xname[sheet]:
                    pl.plot(data[sheet][xname[sheet]][2], data[sheet][species][2],
                            color=colours(data[sheet][species][0]), alpha=alpharamp[frame - 1], linewidth=lw)
    ...
    if magni == 1:
        ...
        if magn <= 2.:
            pl.text(xbound[1] * 0.95, ymax * 0.95, str(round(magn, 1)) + u'×', color='k', horizontalalignment='right',
                    verticalalignment='center', alpha=1.0, fontsize=fs, **font)
        if magn > 2.:
            pl.text(xbound[1] * 0.95, ymax * 0.95, str(round(magn, 0))[:-2] + u'×', color='k',
                    horizontalalignment='right', verticalalignment='center', alpha=1.0, fontsize=fs, **font)
    ...
    if frame >= 1:
        frame = 1000 + frame
        pl.savefig(os.getcwd() + r'\imgs\zoom' + str(timeslot) + r'\frame' + str(frame)[1:4] + '.png',
                   figsize=(19.2, 10.8), dpi=200)
        ...
        for ind1, row in enumerate(rd.rows):
            if ind1 == 1:  # colour definition row
                ...
                try:
                    ...
                    sys.exit(
                        '\nA non-integer value was encountered in Row 3, Column %s of %s (%s).\nEnsure that all values in row 3 are integers.' % (
                        alpha(ind).upper(), sheet, rd.cell(row=ind1 + 1, column=(ind + 1)).value))
            elif ind1 > 2:  # data
                try:
                    data[sheet]['%s' % str(col[0].value)][2].append(
                        float(rd.cell(row=(ind1 + 1), column=(ind + 1)).value))
                except ValueError:
                    sys.exit(
                        '\nA invalid entry was encountered in Row %i, Column %s of %s (%s).\nNumbers are expected in any rows 4 or above.' % (
                        ind1 + 1, alpha(ind).upper(), sheet, rd.cell(row=(ind1 + 1), column=(ind + 1)).value))
...
for ind, val in enumerate(zoomdurs):
    ...
    sys.stdout.write('\rProcessing hold #%i Elapsed time: %ss' % (ind + 1, second_to_str(current_time - time_zero)))
    sys.stdout.flush()
    ...
    zoomramp = linmag(yvalues[0], mag(yvalues[0], yvalues[ind]), mag(yvalues[0], yvalues[ind + 1]),
                      int(zoomdurs[ind] * fps))
    # start,end,duration
    alpharamp = linramp(1, 0, int(zoomdurs[ind] * fps))
    # render ramp frames
    for frame, zoomval in enumerate(zoomramp):
        current_time = time.time()
        sys.stdout.write('\rProcessing zoom #%i (frame #%i/%i %.1f%%) Elapsed time: %ss' % (
        ind + 1, frame + 1, fps * val, float((frame + 1) / (fps * val) * 100), second_to_str(current_time - time_zero)))
        sys.stdout.flush()
...
current_time = time.time()
sys.stdout.write('\rProcessing final hold #%i Elapsed time: %ss' % (ind + 1, second_to_str(current_time - time_zero)))
sys.stdout.flush()
...
gc.collect()
sys.stdout.write('\nEnd time: %s\n' % (time.strftime('%I:%M:%S %p')))
sys.stdout.write('\rElapsed time: %ss' % (second_to_str(time.time() - time_zero)))
