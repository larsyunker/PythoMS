...
def spectrumtrace(filename, sp, scr='all', n=1, mz='all', inj=0., save=1):
    ...
    def plotit(x, y, index):
        ...
        for key in sp:  # ind,val in enumerate(sp):
            l, r = bisect_left(x, sp[key]['bounds'][0]), bisect_right(x, sp[key]['bounds'][
        ...
        for mode in mskeys:
            ...
            for key in sp:
                if sp[key]['affin'] is mode:  # pair species with appropriate rtime
                    axr.plot(rtime[sumkey][:index], sp[key][spkey][:index], linewidth=1.0, label=key,
                             color=sp[key]['colour'])
        ...
        for key in timepoints:  # add vertical timepoint lines
            if maxtime + inj >= timepoints[key]:  # and rtime[0] <= timepoints[ind][1]
                ...
                pl.text(timepoints[key] - inj, 0.5, key, fontsize=fs, color='b', backgroundcolor='w',
                        rotation='vertical', horizontalalignment='center', verticalalignment='center', alpha=0.75,
                        **font)
        ...
        pl.subplots_adjust(left=0.07, right=0.99, bottom=0.095, top=0.96, wspace=0.06,
                           hspace=0.05)  # hard coded subplot tightening
    ...
    sp, TIC, rtime = mzml.pull_species_data(sp)  # integrate species
    ...
    for ind, val in enumerate(spec):
        ...
        if curspec >= scr[0] and curspec <= scr[1]:  # if index is within scanrange to output
            sys.stdout.write('\rRendering scan #%i %.1f%% (scan range: %i to %i)' % (
            curspec, (float(curspec) - float(scr[0])) / (float(scr[1]) - float(scr[0])) * 100., scr[0], scr[1]))
