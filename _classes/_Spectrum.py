...
class Spectrum(object):
    ...
    def __add__(self,x):
        ...
        if isinstance(x,self.__class__) is True: # if it is another Spectrum instance
            ...
            tempspec = Spectrum(self.decpl,start=newstart, end=newend, specin=[self.x,self.y],empty=self.kw['empty'], filler=self.filler) # temporary instance with self as specin
    ...
    def __sub__(self,x):
        if isinstance(x,self.__class__) is True: # if it is another Spectrum instance
            ...
            newstart = min(self.start, x.start) # find new start m/z
            newend = max(self.end, x.end) # find new end m/z
            tempspec = Spectrum(self.decpl,start=newstart, end=newend, specin=[self.x,self.y],empty=self.kw['empty'], filler=self.filler) # temporary instance
    ...
    def addelement(self,masses,abunds):
        ...
        **Note**
        ...
        if len(masses) != len(abunds):
            raise ValueError('The dimensions of the supplied lists are not equal (length masses: %d, length abundances: %d)' %(len(masses),len(abunds)))
        #xboxed = [[0]]
        #yboxed = [[1]]
        xboxed = []
        yboxed = []
    ...
    def consolidate(self,threshold,within,method='abs'):
        ...
        **Parameters**
        ...
        def adjacent(index):
            ...
            if index != len(self.x)-1 and self.x[index+1] - self.x[index] <= within: # if next index is nearer than within
                if self.y[index+1] != 0: # if the peak has intensity
    ...
    def fullspeclist(self,start,end):
        ...
        **Notes**
        ...
        if self.kw['empty'] is False:
            x = self.sp.arange(start,end+10**-self.decpl,10**-self.decpl).tolist() # generate x values using arange and convert to list
    ...
    def index(self,xval):
        ...
        **Notes**
        ...
        if xval > self.end or xval < self.start:
            raise ValueError('the m/z value ({}) is outside of the m/z range of this spectrum ({}-{})'.format(xval,self.start,self.end))
