...
class Spectrum(object):
            ...
            try:  # try indexing
                ...
                if self.empty is False:
                    ...
                    if self.x[index] != round(xval, self.decpl):  # if the index does not equal the value
                        self.x = self.sp.insert(self.x, index,
                                                round(xval, self.decpl))  # insert x value at specified index
    ...
    def fullspeclist(self, start, end):
        ...
        if self.empty is False:
            x = self.sp.arange(start, end + 10 ** -self.decpl,
                               10 ** -self.decpl)  # generate x values using arange and convert to list
    ...
    def index(self, mzval, method='search'):
        ...
        if mzval > self.end or mzval < self.start:
            raise ValueError(
                'the m/z value ({}) is outside of the m/z range of this spectrum ({}-{})'.format(mzval, self.start,
                                                                                                 self.end))
        else:
            """
