...
class mzML(object):
    def __init__(self, filename, **kwargs):
        ...
        self.ks.update(kwargs)  # update defaults with provided keyword arguments
    ...
    def _foreachchrom(self, fn):
        ...
        def foreachchrom(*args, **kwargs):
            ...
            for chromatogram in self.tree.getElementsByTagName('chromatogram'):
                ...
                if self.ks['verbose'] is True:
                    self.sys.stdout.write('\rApplying function "%s" to chromatogram #%d/%d %.1f%%' % (
                    fn.__name__, current, self.chroms, float(current) / float(self.nchroms) * 100.))
    ...
    def _foreachscan(self, fn):
        ...
        def foreachscan(*args, **kwargs):
            ...
            for spectrum in self.tree.getElementsByTagName('spectrum'):
                ...
                if self.ks['verbose'] is True:
                    self.sys.stdout.write('\rApplying function "%s" to scan #%d/%d %.1f%%' % (
                    fn.__name__, current, self.nscans, float(current) / float(self.nscans) * 100.))
        ...
        for ind, rng in enumerate(ranges):
            if self.ks['verbose'] is True:
                self.sys.stdout.write(
                    '\rEstimating resolution of the instrument %.0f%%' % (float(ind + 1) / float(n) * 100.))
    ...
    def check_for_file(self, fn):
        ...
        def version_input(string):
            ...
            if sys.version.startswith('2.7'):
                raise SystemError('Input from Python 2.7 is no longer supported.')
    ...
    def function_timetic(self):
        ...
        for spectrum in self.tree.getElementsByTagName('spectrum'):
            ...
            if self.ks['verbose'] is True:
                self.sys.stdout.write('\rExtracting timepoints and total ion current values from mzML %.1f%%' % (
                        float(attr['index'] + 1) / float(self.nscans) * 100.))
    ...
    def integrate(self, name, start, end, x, y):
        ...
        if end > max(x):  # check that end is within the m/z bounds
            self.BE.warn(name, start, end, min(x), max(x))
        return sum(y[self.locate_in_list(x, start, 'greater'):self.locate_in_list(x, end,
                                                                                  'lesser')])  # integrate using the nearest values inside the bounds
    ...
    def pull_chromatograms(self):
        ...
        for chromatogram in self.tree.getElementsByTagName('chromatogram'):
            ...
            if self.ks['verbose'] is True:
                self.sys.stdout.write('\rExtracting chromatogram #%s/%i  %.1f%%' % (
                attr['index'] + 1, self.nchroms, float(attr['index'] + 1) / float(self.nchroms) * 100.))
                self.sys.stdout.flush()
    ...
    def pullmsmsspectra(self):
        ...
        """
        self.sys.exit(
            'The pullmsmsspectra function is obsolete. Identify the appropriate function to sum from mzML.functions and use retrieve_scans instead.')
    ...
    def pullscansfromfn(self, fn, scan=None, endscan=None):
        ...
        """
        self.sys.exit(
            'The pullscansfromfn function is obsolete. '
            'Identify the appropriate function to sum from mzML.functions and use retrieve_scans instead.')
    ...
    def pull_species_data(self, sp, sumspec=False):
        ...
        for species in sp:  # look for and assign function affinity
            sp[species]['function'] = self.associate_to_function(
                dct=sp[species]
            )  # associate each species in the spectrum with a function
        ...
        for spectrum in self.tree.getElementsByTagName('spectrum'):
            ...
            if self.ks['verbose'] is True:
                self.sys.stdout.write('\rExtracting species data from spectrum #%d/%d  %.1f%%' % (
                attr['index'] + 1, self.nscans, float(attr['index'] + 1) / float(self.nscans) * 100.))
    ...
    def pullspectra(self, sr=None, tr=None, mzrange=None, mute=False):
        ...
        """
        self.sys.exit(
            'The pullspectra function is obsolete. Identify the appropriate function to sum from mzML.functions and use retrieve_scans instead.')
    ...
    def pulluvspectra(self, sr=None, tr=None):
        ...
        """
        self.sys.exit(
            'The pulluvspectra function is obsolete. Identify the appropriate function to sum from mzML.functions and use retrieve_scans instead.')
    ...
    def pw_convert(self, filename, bit=64, compression=True, gzip=True):
        ...
        locs = []
        for val in [  # searches for msconvert.exe in expected folders
            'c:\\program files\\proteowizard',
            'c:\\program files (x86)\\proteowizard',
        ]:
    ...
    def retrieve_scans(self, start=None, end=None, fn=1, mute=False):
        ...
        for spectrum in self.tree.getElementsByTagName('spectrum'):  # go through each spectrum
            ...
            if self.ks['verbose'] is True and mute is False:
                self.sys.stdout.write('\rExtracting scan data from spectrum #%d/%d  %.1f%%' % (
                attr['index'] + 1, self.nscans, float(attr['index'] + 1) / float(self.nscans) * 100.))
    ...
    def scan_index(self, scan=None, fn=1, bias='lesser'):
        ...
        if type(scan) is float:  # timepoint
            if self.ks['ftt'] is False:
                self.function_timetic()
            return self.locate_in_list(
                self.functions[fn]['timepoints'],
                scan,
                bias=bias
            ) + self.functions[fn]['sr'][0]  # return located index plus start of the scan range
        ...
        for acc in p:
            ...
            if acc in othertypes:  # if the scan is something else
                ...
                if 'MS:1000804' in p:  # if it is a UV-Vis
                    ...
                    raise KeyError(
                        'The script has not been coded to handle spectra types other than MS and UV-Vis. Please contact the authors to get this functionality included.')
        return out
    ...
    def sum_scans(self, start=None, end=None, fn=1, dec=3, mute=False):
        ...
        for spectrum in self.tree.getElementsByTagName('spectrum'):  # go through each spectrum
            ...
            if attr['index'] > end:
                break
            if self.ks['verbose'] is True and mute is False:
                self.sys.stdout.write('\rCombining spectrum #%d (scan range: %d-%d)  %.1f%%' % (
                attr['index'] + 1, start, end, (float(attr['index'] - start)) / (float(end - start)) * 100.))
