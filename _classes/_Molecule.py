...
class Molecule(object):
    ...
    def calculate(self):
        ...
        self.rawip = self.rawisotopepattern(self.comp,
                                            dec=self.ks['decpl'])  # generates a raw isotope pattern (charge of 1)
        self.barip = self.barisotopepattern(self.rawip,
                                            self.ks['charge'])  # bar isotope pattern based on the generated raw pattern
    ...
    def composition(self, formula):
            ...
            if formula[0].isupper() is True:  # element is recognized by an uppercase letter
                ...
                for ind, val in enumerate(formula):
                    if formula[ind].isalpha() is True:  # if isotope encountered, return that isotope with n=1
                        return '', {formula: 1}
                self.ks['charge'], self.ks['sign'] = self.interpretcharge(
        ...
        def aas(dic):
            ...
            """
            from PythoMS._classes._formabbrvs import aminoacids  # import dictionary of one-letter amino acid abbreviations
            comptemp = {}
            for key in dic:
    ...
    def gaussianisotopepattern(self):
        ...
        self.nsp = Spectrum(3, min(self.barip[0]) - self.fwhm * 2, max(
            self.barip[0]) + self.fwhm * 2)  # generate Spectrum object to encompass the entire region
    ...
    def isotope(self, string):
        ...
        try:
            ...
            raise ValueError(
                'The element "%s" could not be found in either the predefined common abbreviations, in the NIST element database, nor interpreted as an isotope, please check your input.' % (
                string))
    ...
    def plotgaus(self, exp=None):
        ...
        if exp is not None:  # plots experimental if supplied
            ...
            pl.plot(exp[0], exp[1])
            pl.text(max(exp[0]), 95, 'SER: %s' % comp)
    ...
    def rawisotopepattern(self, comp, thresh=0.01, dec=5):
        ...
        for key in comp:  # for each element
            if key in self.md:  # if natural abundance
                bnds = []
                        ...
                        if mass != 0:
                            if self.md[key][mass][1] != 0:  # if intensity is nonzero
                                for ind, val in enumerate(
                                        out[0]):  # for every current mass in the building isotope pattern
                                    spec.addvalue(out[0][ind] + round(self.md[key][mass][0], dec),
                                                  out[1][ind] * self.md[key][mass][
                                                      1])  # add intensity at the appropriate mass
