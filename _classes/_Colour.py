...
class Colour(object):
    ...
    def interpretstring(self,string):
        ...
        if colourdict.has_key(string) is True:
    ...
    def validatecmyk(self,tup):
        ...
        for val in tup:
            ...
            if val > self.cmyk_scale or val < 0:
                raise ValueError('One of the values in the CMYK colour tuple (%s) is outside of the valid range (0-%d)' %(str(val),self.cmyk_scale,self.__class__.__doc__))
    
    def validatergb(self,tup):
        """checks that values in an RGB tuple are valid"""
        for val in tup:
            if type(val) != int:
                raise ValueError('One of the values in the RGB colour tuple is not an integer ("%s").\nPlease correct the value.\n%s' %(str(val),self.__class__.__doc__))
            elif val > self.rgb_scale or val < 0:
                raise ValueError('One of the values in the RGB colour tuple (%s) is outside of the valid RGB range of 0-%d.\n%s' %(str(val),self.rgb_scale,self.__class__.__doc__))
