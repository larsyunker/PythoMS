        ...
        if peaks.size > 1 and min_dist is not None:  # if there are peaks and a minimum distance has been supplied
            ...
            for peak in highest:
                if not rem[peak]:  # if the peak hasn't already been looked at
                    ind = x[peak]
                    l, r = max(0, np.searchsorted(x, ind - min_dist)), min(len(y) - 1, np.searchsorted(x,
                                                                                                       ind + min_dist))  # find slice based on x values and min_dist
    ...
    if save is True:
        ...
        for ind, val in enumerate(inds):  # write differences based on cell references
            for ind2, val2 in enumerate(inds):
                cs[xlfile.inds_to_cellname(1 + ind, 6 + ind2)] = '=%s-%s' % (
                xlfile.inds_to_cellname(val, 0), xlfile.inds_to_cellname(val2, 0))  # value
